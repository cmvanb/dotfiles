...
sway::install () {
    ...
    if [[ $host == "casino" ]]; then
        force_link "$base_dir/config/sway/outputs.conf~home-laptop" "$XDG_CONFIG_HOME/sway/outputs.conf"
        force_link "$base_dir/config/sway/workspace.conf~home-laptop" "$XDG_CONFIG_HOME/sway/workspace.conf"
